...
# ═════════════════════════════════════════════════════════════════════════════
#  ترقية ١ + ٢: محرّك التطبيع والمقارنة التقريبية (Fuzzy Matching)
# ═════════════════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════════════════
#  ترقية ٣: call_gemini_brand — إعادة محاولة ذكية، البحث محفوظ دائماً
# ═════════════════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════════════════
#  ترقية ٤: build_output_excel — توافق صارم مع منصة سلة
# ═════════════════════════════════════════════════════════════════════════════
...
def build_output_excel(result: dict, original_df: pd.DataFrame, template_bytes: bytes) -> bytes:
    ...
    for t in result.get('testers_to_add', []):
        ...
        if base_r is not None and cat_col:
            cat_val = _clean_cell(base_r.get(cat_col, ''))
        if cat_col:
            if cat_val:
...
def _run_one(b_idx):
    """يُشغّل دفعة واحدة. لم يعد هناك fallback إلى use_grounding=False —
    لأن call_gemini_brand صارت تُعيد المحاولة داخلياً والبحث محفوظ دائماً."""
    with status_lock:
